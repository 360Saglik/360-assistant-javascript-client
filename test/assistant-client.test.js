...
describe('AssistantClient', () => {
    ...
    it('should throw error when params is null in preInstantDoctorOrder', async () => {
        const client = new src_1.AssistantClient({ clientId: CLIENT_ID, secretKey: CLIENT_SECRET });
        await expect(client.preInstantDoctorOrder(null)).rejects.toThrow('params cannot be null');
    });

    it('should throw error when params is null in postInstantDoctorOrder', async () => {
        const client = new src_1.AssistantClient({ clientId: CLIENT_ID, secretKey: CLIENT_SECRET });
        await expect(client.postInstantDoctorOrder(null)).rejects.toThrow('params cannot be null');
    });
